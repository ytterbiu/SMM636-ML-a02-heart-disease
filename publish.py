...
Usage:
    ...
    python publish.py --name output.ipynb    # custom output filename
    ...
    .venv/bin/python3 publish.py python/tmp.ipynb
    .venv/bin/python3 publish.py --push
...
CONFIG = {
    ...
    "notebook": "ml-smm636-a02-private.ipynb",
    # Custom output filename (None = use original notebook name)
    "output_name": "ml-smm636-a02-heart-disease.ipynb",
    # Local path to your cloned public repo
    # "public_repo_path": "ml-smm636-a02-heart-disease.ipynb",
    "public_repo_path": ".",
    ...
    # Whether to strip outputs (True = clean slate for readers)
    "strip_outputs": False,
    # Whether to regenerate requirements.txt
    "generate_requirements": False,  # takes too long
    # Whether to auto-commit after copying
    "auto_commit": False,
    # Whether to auto-commit and push the public repo after copying
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--name",
        default=None,
        help="Custom output filename (e.g. 'my-public-notebook.ipynb')",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
...
def get_output_name(original_path: Path) -> str:
    """Return the final output filename, using custom name if set."""
    ...
    if mode == "blocklist":
        ...
        for cell in nb.cells:
            ...
            if cell_tags.intersection(keep_tags):
                ...
                filtered.append(cell)  # keep markdown by default
        nb.cells = filtered
    return nb, original_count - len(nb.cells)


def strip_cell_metadata(nb):
    """Remove tags and execution_count from every cell."""
    for cell in nb.cells:
...
def strip_and_clean(notebook: Path) -> Path:
    ...
    # Work on a temp copy so the original is untouched
    tmp = notebook.with_name(f"_tmp_{notebook.name}")
    shutil.copy(notebook, tmp)
    # Step 1: Filter cells before nb-clean strips tags
    ...
    strip_cell_metadata(nb)
    print("  ✓ Stripped tags and execution_count from cells.")
...
def copy_to_public(cleaned: Path, public_dir: Path, notebook_path: Path):
    ...
    output_name = get_output_name(notebook_path)
...
def main():
    ...
    if output_name != notebook_path.name:
        print(f"  Output as:  {output_name}")
    ...
    copy_to_public(cleaned, public_dir, notebook_path)
    if CONFIG["auto_commit"]:
        push_public_repo(public_dir, output_name)
    print(f"\n✓ Done! {notebook_path.name} → {output_name}\n")
